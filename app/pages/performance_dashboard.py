...
def add_total_row(df, sum_columns, avg_columns=[], label_column='location'):
...
dbc.Row([
    ...
    dbc.Col(
        ...
        html.H2(
            ...
            "ABC Jewellers Performance Dashboard",
...
dbc.Row([
    ...
    dbc.Col([
        ...
        html.Label("RM", style={"fontSize": "12px"}),
    ...
    dbc.Col([
        ...
        html.Label("ZM", style={"fontSize": "12px"}),
    ...
    dbc.Col([
        ...
        html.Label("Location", style={"fontSize": "12px"}),
        ...
        dcc.Dropdown(
            ...
            options=[
                ...
                for i in location_df_dropdown['location']
            ],
...
def update_dashboard(
    ...
    nsv_g = get_nsv_gauge(start_date, end_date, rm, zm, location)
    gold_g = get_gold_gauge(start_date, end_date, rm, zm, location)
    diamond_g = get_diamond_gauge(start_date, end_date, rm, zm, location)
    silver_g = get_silver_gauge(start_date, end_date, rm, zm, location)
    gemstone_g = get_gemstone_gauge(start_date, end_date, rm, zm, location)
    mohor_g = get_mohor_gauge(start_date, end_date, rm, zm, location)
    ...
    nsv_table = get_nsv_performance(start_date, end_date, rm, zm, location)
    ...
    gold_table = get_gold_performance(start_date, end_date, rm, zm, location)
    ...
    diamond_table = get_diamond_performance(start_date, end_date, rm, zm, location)
    ...
    silver_table = get_silver_performance(start_date, end_date, rm, zm, location)
    ...
    gemstone_table = get_gemstone_performance(start_date, end_date, rm, zm, location)
    ...
    mohor_table = get_mohor_performance(start_date, end_date, rm, zm, location)
    ...
    mc_table = get_making_charge_performance(start_date, end_date, rm, zm, location)
    ...
    scheme_table = get_scheme_performance(start_date, end_date, rm, zm, location)
    ...
    nsv_table['achievement_pct'] = nsv_table['achievement_pct'].apply(
        lambda x: f"{x:.2f}%"
    )

    gold_table['achievement_pct'] = gold_table['achievement_pct'].apply(
        lambda x: f"{x:.2f}%"
    )

    silver_table['achievement_pct'] = silver_table['achievement_pct'].apply(
        lambda x: f"{x:.2f}%"
    )

    diamond_table['achievement_pct'] = diamond_table['achievement_pct'].apply(
        lambda x: f"{x:.2f}%"
    )

    gemstone_table['achievement_pct'] = gemstone_table['achievement_pct'].apply(
        lambda x: f"{x:.2f}%"
    )

    mohor_table['achievement_pct'] = mohor_table['achievement_pct'].apply(
        lambda x: f"{x:.2f}%"
    )
    ...
    for col in nsv_numeric_cols:
        ...
        nsv_table[col] = nsv_table[col].apply(
            lambda x: monetary_format(x, value_format)
        )
    ...
    for col in gold_numeric_cols:
        ...
        gold_table[col] = gold_table[col].apply(
    ...
    for col in silver_numeric_cols:
        ...
        silver_table[col] = silver_table[col].apply(
            lambda x: indian_format(x, 2)
        )
    ...
    for col in diamond_numeric_cols:
        ...
        if 'nsv' in col:
            ...
            diamond_table[col] = diamond_table[col].apply(
                lambda x: monetary_format(x, value_format)
            )
    ...
    for col in gemstone_numeric_cols:
        ...
        gemstone_table[col] = gemstone_table[col].apply(
            lambda x: monetary_format(x, value_format)
        )
    ...
    for col in mohor_2_cols:
        ...
        mohor_table[col] = mohor_table[col].apply(
            lambda x: indian_format(x, 2)
        )
    ...
    for col in mc_numeric_cols:
        ...
        mc_table[col] = mc_table[col].apply(
            lambda x: monetary_format(x, value_format)
        )
    ...
    for col in scheme_0_cols:
        ...
        scheme_table[col] = scheme_table[col].apply(
            lambda x: monetary_format(x, value_format)
        )
