...
# R2 Config from environment variables only
...
def _log_activity_to_r2(email, dashboard_name, ip_address):
    with r2_lock:
        ...
        try:
            ...
            df = pd.DataFrame(columns=["user_email", "dashboard_name", "opened_at", "ip_address"])
...
def log_activity(email, dashboard_name):
    ...
    thread = threading.Thread(
        ...
        args=(email, dashboard_name, ip_address)
    )
    thread.daemon = True
    thread.start()
